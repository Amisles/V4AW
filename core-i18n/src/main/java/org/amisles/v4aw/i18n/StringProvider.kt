...
interface StringProvider {
    ...
    // Site Rules
    val siteRules get() = get(StringKey.SITE_RULES)
    val siteRulesManage get() = get(StringKey.SITE_RULES_MANAGE)
    val siteRuleAdd get() = get(StringKey.SITE_RULE_ADD)
    val siteRuleEdit get() = get(StringKey.SITE_RULE_EDIT)
    val siteRuleName get() = get(StringKey.SITE_RULE_NAME)
    val siteRuleUrlPattern get() = get(StringKey.SITE_RULE_URL_PATTERN)
    val siteRulePriority get() = get(StringKey.SITE_RULE_PRIORITY)
    val siteRuleEnabled get() = get(StringKey.SITE_RULE_ENABLED)
    val siteRuleDisabled get() = get(StringKey.SITE_RULE_DISABLED)
    val siteRuleSave get() = get(StringKey.SITE_RULE_SAVE)
    val siteRuleDelete get() = get(StringKey.SITE_RULE_DELETE)
    val siteRuleDeleteConfirm get() = get(StringKey.SITE_RULE_DELETE_CONFIRM)
    val siteRuleNoRules get() = get(StringKey.SITE_RULE_NO_RULES)
    val siteRuleNoRulesHint get() = get(StringKey.SITE_RULE_NO_RULES_HINT)
    val siteRuleVideoSource get() = get(StringKey.SITE_RULE_VIDEO_SOURCE)
    val siteRuleVideoEntry get() = get(StringKey.SITE_RULE_VIDEO_ENTRY)
    val siteRuleSearchEndpoint get() = get(StringKey.SITE_RULE_SEARCH_ENDPOINT)
    val siteRuleWebviewConfig get() = get(StringKey.SITE_RULE_WEBVIEW_CONFIG)
    val siteRuleCssSelectors get() = get(StringKey.SITE_RULE_CSS_SELECTORS)
    val siteRuleCustomAttributes get() = get(StringKey.SITE_RULE_CUSTOM_ATTRIBUTES)
    val siteRuleScriptUrlPatterns get() = get(StringKey.SITE_RULE_SCRIPT_URL_PATTERNS)
    val siteRuleIframeUrlParams get() = get(StringKey.SITE_RULE_IFRAME_URL_PARAMS)
    val siteRuleContainerSelector get() = get(StringKey.SITE_RULE_CONTAINER_SELECTOR)
    val siteRuleLinkSelector get() = get(StringKey.SITE_RULE_LINK_SELECTOR)
    val siteRuleTitleSelector get() = get(StringKey.SITE_RULE_TITLE_SELECTOR)
    val siteRuleThumbnailSelector get() = get(StringKey.SITE_RULE_THUMBNAIL_SELECTOR)
    val siteRuleSearchUrlTemplate get() = get(StringKey.SITE_RULE_SEARCH_URL_TEMPLATE)
    val siteRuleSearchMethod get() = get(StringKey.SITE_RULE_SEARCH_METHOD)
    val siteRulePageLoadDelay get() = get(StringKey.SITE_RULE_PAGE_LOAD_DELAY)
    val siteRuleScrollBeforeExtract get() = get(StringKey.SITE_RULE_SCROLL_BEFORE_EXTRACT)
    val siteRuleCustomUserAgent get() = get(StringKey.SITE_RULE_CUSTOM_USER_AGENT)
    val siteRuleDisableAdBlock get() = get(StringKey.SITE_RULE_DISABLE_AD_BLOCK)
    val siteRuleNameRequired get() = get(StringKey.SITE_RULE_NAME_REQUIRED)
    val siteRulePatternRequired get() = get(StringKey.SITE_RULE_PATTERN_REQUIRED)
    val siteRuleSaved get() = get(StringKey.SITE_RULE_SAVED)
